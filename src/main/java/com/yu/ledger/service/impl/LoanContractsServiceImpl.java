...
import static com.yu.ledger.entity.enums.ContractStatusEnum.ACTIVE;
import static com.yu.ledger.entity.enums.ContractStatusEnum.CLOSED;
...
public class LoanContractsServiceImpl implements ILoanContractsService {
    ...
    public List<Map<String, Object>> listContracts(Map<String, Object> params) {
        LambdaQueryWrapper<LoanContracts> wrapper = new LambdaQueryWrapper<>();
        // 关键字（合同号/合同名/客户名）
        if (params.get("keyword") != null && !params.get("keyword").toString().trim().isEmpty()) {
            ...
            if (!contractNameList.isEmpty()) {
                ...
                wrapper.in(LoanContracts::getContractId, contractIds);
            } else {
                // 再查客户名
                ...
                if (!customers.isEmpty()) {
                    ...
                    wrapper.in(LoanContracts::getCustomerId, customerIds);
                } else {
                    // 最后查合同ID
                    try {
                        Integer contractId = Integer.valueOf(keyword);
                        wrapper.eq(LoanContracts::getContractId, contractId);
                    } catch (NumberFormatException e) {
                        wrapper.eq(LoanContracts::getContractId, -1);
                    }
                }
            }
        }
        ...
        if (params.get("status") != null && !params.get("status").toString().trim().isEmpty()) {
            wrapper.eq(LoanContracts::getContractStatus, params.get("status"));
        }
        // 还款频率
        if (params.get("frequency") != null && !params.get("frequency").toString().trim().isEmpty()) {
            wrapper.eq(LoanContracts::getRepaymentFrequency, params.get("frequency"));
        }
        wrapper.orderByDesc(LoanContracts::getContractId);

        return loanContractsMapper.selectList(wrapper).stream()
                .map((c -> {
    ...
    public Map<String, Object> closeContract(String contractId) {
        ...
        entity.setContractStatus(CLOSED.getCode());
        loanContractsMapper.updateById(entity);
        // 删除该合同下所有还款计划
        repaymentScheduleMapper.delete(new LambdaQueryWrapper<RepaymentSchedule>()
    ...
    public List<Map<String, Object>> getContractSchedules(String contractId) {
        ...
        return repaymentScheduleMapper.selectList(new LambdaQueryWrapper<RepaymentSchedule>()
                        .eq(RepaymentSchedule::getContractId, contractId)
                        .orderByAsc(RepaymentSchedule::getPeriodStart)).stream()
                .map(schedule -> {
                    Map<String, Object> map = new HashMap<>();
    ...
    public List<Map<String, Object>> contractOptions(Integer customerId) {
        ...
        return  loanContractsMapper.selectList(new LambdaQueryWrapper<LoanContracts>()
                        .eq(customerId != null, LoanContracts::getCustomerId, customerId)).stream()
                .map(c -> {
                    Map<String, Object> map = new HashMap<>();
                    map.put("contract_id", c.getContractId());
                    map.put("contract_name", c.getContractName());
